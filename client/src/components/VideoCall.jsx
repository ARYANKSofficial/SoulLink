...
const VideoCall = ({ roomId }) => {
    ...
    // ----------------------------------------------------------------
    // 3. LOCAL MEDIA SETUP (Used by PC)
    // ----------------------------------------------------------------
    const setupLocalMedia = async (pc) => {
        // Stream should be ready from init, but safety check
        let stream = localStreamRef.current;
        if (!stream) {
            console.log("Local stream not ready yet, waiting...");
            // Simple polling wait since we expect init to finish
            for (let i = 0; i < 50; i++) {
        ...
        <div className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                >
                    ...
                    />
                    {!localStream && (
                        <div className="absolute inset-0 flex items-center justify-center text-gray-500 animate-pulse">Initializing Camera...</div>
                    )}
